...
export function openParam(windows: WindowState[]): string {
  ...
  return [...rest, ...(active ? [active] : [])].map((w) => w.id).join(",");
}
...
export const windowUrl = (w: WindowState) => `${window.location.origin}/?open=${w.id}`;
...
export function openLinks(base: WindowState[], links: WindowLink[], vw: number, vh: number): WindowState[] {
  ...
  return links.reduce<WindowState[]>((state, { kind, params }) => {
    ...
    const saved = base.find((s) => s.id === id);
    if (!saved || state.some((s) => s.id === id)) return desktopReducer(state, open);
    return desktopReducer([...state, { ...saved, minimized: false, z: 0 }], { type: "focus", id });
  }, []);
}
